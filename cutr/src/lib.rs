...
pub fn get_args() -> MyResult<Config> {
    ...
    let extract = matches.value_of_lossy("bytes");

    let index = parse_index("10")?;
    println!("The output {}", index);

    return Err("uh oh".into());
    // return Ok(Config {
    //     files: matches.value_of_lossy("file").unwrap(),
    //     delimiter: matches.value_of_lossy("delimiter").unwrap(),
    //     extract: ,
    // });
}
